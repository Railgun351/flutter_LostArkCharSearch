...
import 'charprofile_dto.dart';

class CharProfileRepository {
  static final list = List<ProfileDTO>.empty(growable: true);

  static Future<List<ProfileDTO>> getDTO(List<NameDTO> nameList) async {
    List<Future> futures = [];
    list.clear();
    nameList.forEach((element) {
      final String url =
          'https://developer-lostark.game.onstove.com/armories/characters/${element.characterName}/profiles';

      Future future = http.get(Uri.parse(url), headers: {
        ...
        if (response.statusCode == 200) {
          ...
          list.add(ProfileDTO.fromJson(json));
          print(list.length);
        } else {
